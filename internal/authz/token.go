...
func Middleware(next http.Handler, validateToken func(string) (string, []string, error), logger *slog.Logger) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if authHeader == "" {
			logger.Error("No Authorization header present", "url", r.URL.String())
			next.ServeHTTP(w, r)
			return
		}
		...
		if err != nil {
			logger.Error("Failed to extract bearer token", "error", err, "auth_header", maskAuthHeader(authHeader))
			next.ServeHTTP(w, r)
			return
		}
		...
		if err != nil {
			logger.Error("Failed to validate token", "error", err, "token_length", len(token))
			next.ServeHTTP(w, r)
			return
		}

		if userID == "" {
			logger.Error("Empty user ID from token", "token_length", len(token))
			next.ServeHTTP(w, r)
			return
		}
